...
TOOLS="--tools statistics_analysis evaluate_formula submit_formula polynomial_fit code_executor call_sindy call_pysr predict_property"
EXP="08_all_tools"

echo "[$(date)] START all_tools group (all 7 formulas)"

echo "[$(date)] A: sin(x1-x2)"
conda run -n sragent python run_sr_agent.py -f "y = sin(x1 - x2)" --x-low -10 --x-high 10 $TOOLS $COMMON --exp_name "${EXP}_sinx1x2" 2>&1 | tail -3

echo "[$(date)] B: II.24.17_0_1"
conda run -n sragent python bench_sr_agent.py --algorithm my_sr_agent --datasets lsrtransform --problem_names II.24.17_0_1 $TOOLS $COMMON --exp_name "$EXP" 2>&1 | tail -3

echo "[$(date)] C: II.13.23_1_0"
conda run -n sragent python bench_sr_agent.py --algorithm my_sr_agent --datasets lsrtransform --problem_names II.13.23_1_0 $TOOLS $COMMON --exp_name "$EXP" 2>&1 | tail -3

echo "[$(date)] D: I.34.14_2_0"
conda run -n sragent python bench_sr_agent.py --algorithm my_sr_agent --datasets lsrtransform --problem_names I.34.14_2_0 $TOOLS $COMMON --exp_name "$EXP" 2>&1 | tail -3
